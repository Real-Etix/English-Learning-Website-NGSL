...
export function FeedbackPanel({
  ...
  return (
    <section className="space-y-5 rounded-[2rem] border border-emerald-300/60 bg-emerald-50/90 p-6 shadow-sm">
      <div>
        <div className="text-xs uppercase tracking-[0.2em] text-emerald-800">
          {title}
        </div>
        <h2 className="mt-2 text-3xl font-semibold text-slate-900">{word.lemma}</h2>
        <p className="mt-2 text-sm text-emerald-900/90">
          {word.partOfSpeech} · {word.definition}
        </p>
        <p className="mt-3 text-sm text-emerald-800/85">{subtitle}</p>
        <div className="mt-4">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.3fr_0.9fr]">
        <div className="space-y-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Example sentences</h3>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Useful phrase bundles</h3>
        ...
        <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-lg font-semibold text-slate-900">Real-world usage</h3>
          ...
          {references.slice(0, 2).length > 0 ? (
            <div className="space-y-3">
              {references.slice(0, 2).map((reference) => (
                <div key={reference.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="text-xs uppercase tracking-[0.2em] text-cyan-800">
                    {reference.platform}
                    {reference.timestampLabel ? ` · ${reference.timestampLabel}` : ""}
                  </div>
                  <div className="mt-2 font-medium text-slate-900">{reference.title}</div>
                  <p className="mt-2 text-sm leading-6 text-slate-600">{reference.excerpt}</p>
                </div>
              ))}
            </div>
          ) : (
          ...
          <Link
            ...
            className="inline-flex rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 shadow-sm transition hover:bg-slate-50"
          >
            Open full word page
          </Link>
        </div>
