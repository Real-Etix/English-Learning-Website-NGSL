...
function cacheSegment(value: string, label: string): string {
...
export function verificationCacheKey(input: CacheKeyInput): string {
  const provider = cacheSegment(input.provider, "provider");
...
function cacheEntryPath(root: string, key: string): string {
  ...
  const namespace = cacheSegment(parts[0]!, "cache namespace");
  const digest = cacheSegment(parts[1]!, "cache key");
...
export async function readVerificationCache<T>(
  ...
  try {
    const stored = JSON.parse(await readFile(cacheEntryPath(root, key), "utf8")) as unknown;
...
export async function writeVerificationCache<T>(
  ...
  const directory = join(root, key.split("/")[0]!);
  const temporary = join(directory, `.${entry.split("/").pop()!}.${process.pid}.${randomUUID()}.tmp`);
...
function scopedKey(namespace: string, key: string): string {
  const safeNamespace = cacheSegment(namespace, "cache namespace");
  return key.includes("/") ? key : `${safeNamespace}/${key}`;
}
