---
lemma: inspection
display: inspection
tier: core
pos: noun
forms: [inspection, inspections]
lists: [toeic, business]
rank: 136
sfi: 57.24
sources: [wordnet, llm]
status: enriched
---

## Definition
a formal or official examination

## Examples
- we had to wait for the inspection before we could use the elevator _(wordnet)_

## Connections
- synonym: [[review]]
- domain: toeic
- domain: business
- advanced_form: [[scrutiny]] — Use 'scrutiny' for a very close, critical, and often prolonged inspection, especially when checking for flaws or details.
- advanced_form: [[audit]] — Use 'audit' specifically for a systematic and official examination of financial or operational records, often required by law or regulation.
- advanced_form: [[survey]] — Use 'survey' for a broad and systematic inspection of a physical area, group, or situation to gather information or assess condition.
- builds_on: [[examination]] — Use 'inspection' for an official or systematic examination, especially to check standards.
