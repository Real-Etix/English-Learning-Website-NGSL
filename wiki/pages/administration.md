---
lemma: administration
display: administration
tier: core
pos: noun
forms: [administration, administrations]
lists: [ngsl]
rank: 1046
sfi: 59.36
sources: [wordnet, llm]
status: enriched
---

## Definition
a method of tending to or managing the affairs of a some group of people (especially the group's business affairs)  

## Examples
_No sourced examples yet._

## Connections
- synonym: [[disposal]]
- builds_on: [[government]] — Use 'administration' when referring to the executive branch or the current team in power, especially in the context of a specific period.
- advanced_form: [[governance]] — Use 'governance' when emphasizing the system and structure of control and direction, especially for organizations or institutions.
- advanced_form: [[stewardship]] — Use 'stewardship' when highlighting responsible care and ethical management of resources or duties.
- builds_on: [[presidency]] — Use 'administration' to refer to the entire period and governing body of a president, not just the tenure.
- builds_on: [[enforcement]] — Use 'administration' when referring to the official management and application of laws or punishments, implying a systematic process.
- builds_on: [[bureaucracy]] — Use 'administration' when focusing on the management and executive functions, rather than the impersonal rule-bound system implied by 'bureaucracy'.
