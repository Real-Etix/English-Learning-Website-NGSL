---
lemma: induce
display: induce
tier: core
pos: verb
forms: [induce, induces, induced, inducing]
lists: [business, academic]
rank: 22
sfi: 61.652
sources: [wordnet, llm]
status: enriched
---

## Definition
cause to arise

## Examples
- induce a crisis _(wordnet)_

## Connections
- domain: business
- domain: academic
- advanced_form: [[elicit]] — Use 'elicit' when the cause is indirect, often drawing out a reaction or response.
- advanced_form: [[provoke]] — Use 'provoke' when the action intentionally or unintentionally stirs up a strong emotional or behavioral reaction.
- advanced_form: [[engender]] — Use 'engender' to suggest a gradual or intrinsic creation of a feeling or situation, often more formal and abstract.
- builds_on: [[render]] — Use 'induce' when emphasizing that something causes a state or condition, often with a sense of bringing it about actively, while 'render' is more neutral and focuses on the result.
- builds_on: [[convince]] — Use 'induce' when the convincing involves a cause-and-effect relationship, often with external factors or incentives.
