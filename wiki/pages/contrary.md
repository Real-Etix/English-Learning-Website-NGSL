---
lemma: contrary
display: contrary
tier: core
pos: adjective
forms: [contrary, contraries]
lists: [toeic, business, academic]
rank: 252
sfi: 57.084
sources: [wordnet, llm]
status: enriched
---

## Definition
very opposed in nature or character or purpose

## Examples
- acts contrary to our code of ethics _(wordnet)_
- the facts point to a contrary conclusion _(wordnet)_

## Connections
- domain: toeic
- domain: business
- domain: academic
- advanced_form: [[antithetical]] — Use when emphasizing a direct and complete opposition, often in abstract or philosophical contexts.
- advanced_form: [[antagonistic]] — Use when the opposition is active, hostile, or confrontational rather than just different.
- advanced_form: [[adverse]] — Use when the opposition is harmful or unfavorable, especially in circumstances or conditions.
- builds_on: [[negative]] — Use 'contrary' instead of 'negative' when emphasizing active opposition or contradiction, often in debates or writing.
- builds_on: [[opposite]] — Use 'contrary' when emphasizing active disagreement or opposition in views or actions.
