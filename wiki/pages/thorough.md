---
lemma: thorough
display: thorough
tier: core
pos: adjective
forms: [thorough]
lists: [toeic, business]
rank: 262
sfi: 55.57
sources: [wordnet, llm]
status: enriched
chart: thorough
region: you
---

## Definition
painstakingly careful and accurate

## Examples
- our accountant is thorough _(wordnet)_
- thorough research _(wordnet)_

## Connections
- domain: toeic
- domain: business
- advanced_form: [[meticulous]] — Use 'meticulous' when emphasizing extreme attention to small details, often with a connotation of thoroughness bordering on fussiness.
- advanced_form: [[exhaustive]] — Use 'exhaustive' when the thoroughness is so complete that nothing is left out; implies covering every possible aspect.
- advanced_form: [[rigorous]] — Use 'rigorous' when thoroughness involves strictness, precision, and often intellectual or methodological discipline.
- builds_on: [[careful]] — Use 'thorough' when focusing on completeness and covering all aspects, not just caution.
- builds_on: [[comprehensive]] — Use 'thorough' when focusing on careful, detailed coverage of every part; it implies meticulousness beyond just 'comprehensive'.
- synonym: [[assiduous]]
- synonym: [[conscientious]]
- collocation: [[perusal]]
- collocation: [[rubdown]]
- collocation: [[vetting]]
- synonym: [[intensive]]
