---
lemma: circuit
display: circuit
tier: core
pos: noun
forms: [circuit, circuits, circuited, circuiting]
lists: [toeic, business, fitness]
rank: 304
sfi: 55.68
sources: [wordnet, llm]
status: enriched
---

## Definition
an electrical device that provides a path for electrical current to flow  

## Examples
_No sourced examples yet._

## Connections
- domain: toeic
- domain: business
- domain: fitness
- builds_on: [[itinerary]] — Use 'circuit' when the route returns to the starting point or is a regular loop, unlike 'itinerary' which is a planned list of stops.
- advanced_form: [[channel]] — Use 'channel' when emphasizing the directed flow path, often in semiconductor or signal routing contexts.
- advanced_form: [[conduit]] — Use 'conduit' when referring to the physical enclosure that houses the electrical circuit, especially in building wiring.
- builds_on: [[tour]] — Use 'circuit' instead of 'tour' when the route is round-trip, returning to the starting point.
