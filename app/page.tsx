    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-14 px-6 py-12">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={item} className="rounded-3xl bg-slate-950/50 p-4 text-sm text-slate-200">
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="space-y-6">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl font-semibold text-white">Pick how you feel today</h2>
        </div>
        <MoodSelector />
      </section>
      ...
      <section className="space-y-6">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/dashboard" className="text-sm text-cyan-100 hover:text-white">
            View dashboard
          </Link>
        </div>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        {lists.map((list) => (
          ...
          >
            ...
            <h3 className="mt-3 text-2xl font-semibold text-white">{list.title}</h3>
            <p className="mt-3 text-sm leading-7 text-slate-300">{list.description}</p>
            <div className="mt-5 flex flex-wrap gap-2">
              {list.featuredWords.map((word) => (
                <Link
                  key={`${list.slug}-${word.lemma}`}
                  href={`/word/${word.normalizedLemma}`}
                  className="rounded-full border border-white/10 bg-slate-950/40 px-3 py-2 text-sm text-slate-100 transition hover:bg-white/10"
                >
                  {word.lemma}
                </Link>
              ))}
            ...
            <Link
              ...
              className="mt-6 inline-flex rounded-full border border-cyan-400/30 px-4 py-2 text-sm text-cyan-100 transition hover:bg-cyan-400/10"
            >
              Practice this list
            </Link>
          </article>
